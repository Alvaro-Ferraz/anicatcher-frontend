  ...
  return (
    <section>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-1xl font-sans font-semibold px-5 sm:px-5 text-[#ADC0D2]">{title}</h2>
      </div>
      ...
      <div className={responsiveGridClass + ' relative z-0 px-5 sm:p-5 overflow-hidden'}>
        {isLoading ? (
          ...
          [...Array(nonLoadingCount)].map((_, index) => {
            ...
            const imgSrc = anime.images?.jpg?.large_image_url || anime.images?.jpg?.image_url || 'https://via.placeholder.com/150';
            ...
            return (
              ...
              >
                <a
                  ...
                  className="block w-full aspect-[3/4] overflow-hidden rounded bg-[#1e2a3a]"
                >
                  <img
                    src={imgSrc}
                    alt={anime.title}
                ...
                {anime.subtitle && <p className="text-gray-400 text-xs mt-1">{anime.subtitle}</p>}
              </div>
            );
          })
        )}
